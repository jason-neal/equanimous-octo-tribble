...
def main():
    ...
    # Convolution to 80k
    R = 80000
    wav_80k, flux_80k = IPconvolution(wav,
                    flux, chip_limits, R, FWHM_lim=5.0, plot=False, verbose=True)

    # Convolution to 50k
    R = 50000
    wav_50k, flux_50k = IPconvolution(wav,
                    flux, chip_limits, R, FWHM_lim=5.0, plot=False, verbose=True)

    wav_80k_50k, flux_80k_50k = IPconvolution(wav_80k,
                    flux_80k, chip_limits, R, FWHM_lim=5.0, plot=False, verbose=True)

    # Convolution to 20k
    R = 20000
    wav_80k_20k, flux_80k_20k = IPconvolution(wav_80k,
                    flux_80k, chip_limits, R, FWHM_lim=5.0, plot=False, verbose=True)

    wav_50k_20k, flux_50k_20k = IPconvolution(wav_50k,
                    flux_50k, chip_limits, R, FWHM_lim=5.0, plot=False, verbose=True)

    wav_80k_50k_20k, flux_80k_50k_20k = IPconvolution(wav_80k_50k,
                    flux_80k_50k, chip_limits, R, FWHM_lim=5.0, plot=False, verbose=True)

    # Convolution straight to 20000
    wav_20k, flux_20k = IPconvolution(wav,
                    flux, chip_limits, R, FWHM_lim=5.0, plot=False, verbose=True)
    ...
    plt.plot(wav_80k, flux_80k / np.max(flux_80k), color='r',
             linestyle="-.", label="R=80k-20k")
    plt.plot(wav_50k, flux_50k / np.max(flux_50k), color='b',
             linestyle="--", label="R=50k")
    ...
    plt.plot(wav_20k, flux_20k / np.max(flux_20k), color='c',
             linestyle="-", label="R=20k")
    plt.legend(loc='best')
